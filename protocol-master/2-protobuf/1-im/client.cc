...
void start(void)
{
	...
	while(1)
	{
		CImPdu cPdu; // protocol data unit
		// if(login_msg % 1 == 0) 
		{
			// 发送登录信息
			IM::Login::IMLoginReq login;
			IM::Login::MusicResponse* mr = new IM::Login::MusicResponse;
			mr->set_author("zjl");
			mr->set_data("123");
			mr->set_title("半岛铁盒");
		
			IM::Login::ArticleResponse* ar = new IM::Login::ArticleResponse;
			ar->set_author("my");
			ar->set_date("2021年10月12日");
			ar->set_title("红高粱家族");

			login.set_user_name("liaoqingfu");
		    login.set_password("123456");	// protobuf编译器生成的接口
		    login.set_online_status(IM::BaseDefine::USER_STATUS_ONLINE);
		    login.set_client_type(IM::BaseDefine::CLIENT_TYPE_WINDOWS);
		   	login.set_client_version("1.0");

			if(login_msg %2 == 0) {
				login.set_string_2("test oneof");
				login.set_allocated_musicresponse(mr);
			}else {
				login.set_allocated_articleresponse(ar);
			}
			
		    cPdu.SetPBMsg(&login);	// 把protobuf序列化后的数据填充到data段
			...
			send(sockfd, cPdu.GetBuffer(), cPdu.GetLength(), 0); // header + body
		}
		sleep(1);
	    // else //if(login_msg % 2 == 0) 	// 发送文本消息
		{
