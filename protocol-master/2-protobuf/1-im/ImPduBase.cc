...
{
    uchar_t* buf = GetBuffer();
    CByteStream::WriteUint16(buf + 8, service_id); // 修改header对应的位置
}

void CImPdu::SetCommandId(uint16_t command_id)
{
    uchar_t* buf = GetBuffer();
    CByteStream::WriteUint16(buf + 10, command_id);// 修改header对应的位置
}
...
void CImPdu::SetPBMsg(const google::protobuf::MessageLite* msg)
{
    ...
    uint32_t msg_size = msg->ByteSize();  // pb对象序列化的大小
    uchar_t* szData = new uchar_t[msg_size];
    //ALLOC_FAIL_ASSERT(szData)
    if (!msg->SerializeToArray(szData, msg_size)) // 序列化pb后的数据
    {
        log_error("pb msg miss required fields.");
    }
    
    m_buf.Write(szData, msg_size);		// 2. 写pb的数据 m_buf= header + body(pb序列化后的数据)
    delete []szData;
    WriteHeader();			// 
}
